import { ProcessingStatus, ExtractedReport, ReportListItem } from "../types/extraction";

const API_BASE = "/api";

export async function uploadPDF(file: File): Promise<{ jobId: string; filename: string }> {
...
export async function deleteReport(id: string): Promise<void> {
  const response = await fetch(`${API_BASE}/upload/reports/${id}`, { method: "DELETE" });
  if (!response.ok) throw new Error("Failed to delete report");
}

export async function selectCounty(reportId: string, county: string): Promise<void> {
  const response = await fetch(`${API_BASE}/upload/reports/${reportId}/county`, {
    method: "PATCH",
...
export async function checkHealth(): Promise<{ status: string; hasOpenAIKey: boolean; hasAnthropicKey: boolean }> {
